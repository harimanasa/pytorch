...
std::vector<c10::optional<IValue>> findEPSWeightBiasConstantsOfBN(
  ...
  if (matches.size() == 0) {
    return std::vector<c10::optional<IValue>>(3, c10::optional<IValue>{});
  }
  const auto& match = matches[0];
  std::vector<c10::optional<IValue>> eps_weight_bias_constants;
  eps_weight_bias_constants.reserve(3);
  eps_weight_bias_constants.emplace_back(toIValue(match.values_map.at(bn_vmap.at("eps"))));
  eps_weight_bias_constants.emplace_back(toIValue(match.values_map.at(bn_vmap.at("weight"))));
  eps_weight_bias_constants.emplace_back(toIValue(match.values_map.at(bn_vmap.at("bias"))));
  return eps_weight_bias_constants;
}
